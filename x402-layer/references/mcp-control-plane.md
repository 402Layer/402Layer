...
Use this reference when the user wants optional dashboard-level access through Omega MCP instead of only local scripts.

## What MCP Is For

Omega MCP is the owner-scoped control plane for:
...
- do not treat `OMEGA_PAT` as globally required for skill installation
...
- if the task is owner-scoped dashboard inventory or configuration, prefer Omega MCP when a PAT is available
