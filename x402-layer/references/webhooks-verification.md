...
Omega Studio seller webhooks are HMAC-signed. Expect these headers on current deliveries:
...
```json
{
  ...
  "data": {
    ...
    "receipt_token": "eyJ...",
    "jwks_url": "https://api.x402omega.cc/.well-known/jwks.json"
  }
}
```
...
Any query parameter on a payment URL that is not a Omega-internal parameter is automatically captured in the webhook `metadata` field. This works like Stripe metadata passthrough.
...
```
https://api.x402omega.cc/e/my-api?order_id=abc-123
```
