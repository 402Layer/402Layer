...
description: |
  ...
  "openwallet.sh", or use optional Omega MCP
  access with a dashboard PAT to manage x402 Omega Layer operations
  on Base, Ethereum, Polygon, BSC, Monad, or Solana networks.
homepage: https://docs.x402omega.cc/agentic-access/openclaw-skill
metadata:
  clawdbot:
    emoji: "⚡"
    homepage: https://studio.x402omega.cc
    os:
      - linux
      - darwin
    requires:
...
This skill covers the full Omega Layer lifecycle:
...
- optionally use Omega MCP for owner-scoped dashboard and control-plane actions
...
>
> **Expected network/binary surface:** `api.x402omega.cc`, `studio.x402omega.cc`, `mcp.x402omega.cc`, and local `awal` / `ows` binaries when those wallet modes are explicitly enabled.
...
If the user provides a dashboard PAT, the agent can also use Omega MCP for owner-scoped account actions:

```bash
export OMEGA_PAT="omega_pat_..."
```
...
- existing dashboard/agent management routes on `api.x402omega.cc`
...
### B) Pay and Consume
```bash
python {baseDir}/scripts/pay_base.py https://api.x402omega.cc/e/weather-data
python {baseDir}/scripts/pay_base.py https://api.x402omega.cc/e/weather-data --agentkit auto
python {baseDir}/scripts/pay_solana.py https://api.x402omega.cc/e/weather-data
python {baseDir}/scripts/consume_credits.py https://api.x402omega.cc/e/weather-data

# Optional OWS backend
python {baseDir}/scripts/ows_cli.py pay-url https://api.x402omega.cc/e/weather-data --wallet hackathon-wallet
```
...
python {baseDir}/scripts/pay_base.py https://api.x402omega.cc/e/weather-data --agentkit required
```
...
# Then use Omega MCP for owner inventory/config operations such as:
# - list_my_endpoints
# - list_my_campaigns
# - create_campaign
# - update_campaign
...
- `references/mcp-control-plane.md`:
  when to use Omega MCP, what PAT scopes are needed, and which owner-scoped actions should prefer MCP over direct scripts.
...
| `X402_API_BASE` | API override | default `https://api.x402omega.cc` |
...
| Variable | Used by | Notes |
|---|---|---|
| `OMEGA_PAT` | Omega MCP owner-scoped management flows | optional PAT in `omega_pat_*` format; not required for install or normal script usage |
...
| `X402_API_BASE_URL` | support thread scripts | default `https://api.x402omega.cc` |
...
- Endpoints: `https://api.x402omega.cc/e/{slug}`
- Marketplace: `https://api.x402omega.cc/api/marketplace`
- Credits: `https://api.x402omega.cc/api/credits/*`
- Agent routes: `https://api.x402omega.cc/agent/*`
- MCP: `https://mcp.x402omega.cc/mcp`
