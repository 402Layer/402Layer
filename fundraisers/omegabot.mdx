---
title: "OmegaBot on X"
description: "Launch fundraisers by tagging @OmegaBot on X — how it works, required info, and claiming"
---

[@OmegaBot](https://x.com/OmegaBot) is the official X bot for Omega Agentic Fundraisers. Tag it on X to launch a fundraiser for anyone — no account required. The bot will collect the required details, create the campaign, and post the link back in the thread.

## How to Launch a Fundraiser

1. **Tag @OmegaBot on X** — Mention `@OmegaBot` in a tweet or reply. Tell it who the fundraiser is for and include as much detail as you can.
...
@OmegaBot launch a fundraiser for @musician $MUSIC target $5000 USDC 7 days
```

```text Minimal (bot asks for rest)
@OmegaBot launch a fundraiser for @musician
```
...
When someone creates a fundraiser for you via @OmegaBot, you need to claim it to verify your identity.

<Steps>
  <Step title="Sign in to Omega Studio">
    Go to [studio.x402omega.cc](https://studio.x402omega.cc) and sign in with any method (email, Google, etc.).
  </Step>
  ...
  <Step title="Visit the Created for You tab">
    Go to [My Fundraisers](https://studio.x402omega.cc/fundraisers/my). Campaigns created for your X handle appear under "Created for You".
  </Step>
  <Step title="Claim">
...
<CardGroup cols={2}>
  ...
  <Card title="Campaign Page" icon="globe">
    Each campaign gets a public page at `studio.x402omega.cc/fundraisers/[slug]` where anyone can contribute USDC.
  </Card>
...
<AccordionGroup>
  <Accordion title="Can I create a fundraiser for myself?">
    Yes. Tag @OmegaBot and say "for me" or "for myself".
  </Accordion>
  ...
  <Accordion title="Can anyone reply in the thread to add info?">
    No. Only the person who originally tagged @OmegaBot can provide follow-up details. Other replies are ignored.
  </Accordion>
  <Accordion title="What if the bot doesn't reply?">
    Make sure @OmegaBot is directly mentioned in the tweet text. Replies in a thread that don't explicitly tag @OmegaBot will not trigger a new campaign.
  </Accordion>
