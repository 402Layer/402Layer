...
homepage: https://studio.x402omega.cc/docs/agentic-access/x402-compute
metadata:
  clawdbot:
    emoji: "🖥️"
    homepage: https://compute.x402omega.cc
    os:
      - linux
      - darwin
    requires:
...
**Base URL:** `https://compute.x402omega.cc`
...
npx mppx https://compute.x402omega.cc/compute/provision \
  -X POST \
...
npx mppx https://compute.x402omega.cc/compute/instances/<instance_id>/extend \
  -X POST \
...
curl -X POST https://compute.x402omega.cc/compute/instances/<instance_id>/resize \
...
curl -X POST https://compute.x402omega.cc/compute/credits/topup \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $COMPUTE_API_KEY" \
...
curl https://compute.x402omega.cc/compute/credits/balance \
  -H "X-API-Key: $COMPUTE_API_KEY"

# Provision using credits (no x402/MPP payment needed)
curl -X POST https://compute.x402omega.cc/compute/provision \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $COMPUTE_API_KEY" \
  -d '{
    "plan": "vc2-1c-1gb",
...
curl -X POST https://compute.x402omega.cc/compute/instances/<instance_id>/extend \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $COMPUTE_API_KEY" \
...
- 📖 **Documentation:** [studio.x402omega.cc/docs/agentic-access/x402-compute](https://studio.x402omega.cc/docs/agentic-access/x402-compute)
- 🖥️ **Compute Dashboard:** [compute.x402omega.cc](https://compute.x402omega.cc)
- 🌐 **Omega Studio:** [studio.x402omega.cc](https://studio.x402omega.cc)
