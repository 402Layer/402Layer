...
Once your enterprise partner account is created, you can access your dashboard at **Dashboard > Enterprise** in Omega Studio.
...
Your platform authenticates with the Omega Enterprise API using API keys. This lets your backend list endpoints, query revenue, and identify your agents during endpoint creation.
...
```bash
curl https://studio.x402omega.cc/api/v1/enterprise/endpoints \
  -H "X-Enterprise-Key: omega_ent_your_key_here"
```
...
For agent-native integration, your agents can connect to the Omega MCP server directly:

**MCP Endpoint:** `https://mcp.x402omega.cc/mcp`
...
X-Enterprise-Key: omega_ent_your_key_here
...
Agents connected to the Omega MCP server include `enterprise_partner_slug` in the tool call to route through your program.
