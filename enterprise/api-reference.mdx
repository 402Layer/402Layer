...
description: REST API for enterprise partners to integrate Omega Layer into their platform
---

# Enterprise API Reference

The Enterprise API lets you integrate Omega Layer directly into your platform. Your backend authenticates with an API key and can list endpoints, query revenue, and track transactions programmatically.
...
```bash
curl https://studio.x402omega.cc/api/v1/enterprise \
  -H "X-Enterprise-Key: omega_ent_your_api_key_here"
```

### Getting an API Key

1. Go to your [Enterprise Dashboard](https://studio.x402omega.cc/enterprise/dashboard)
2. Navigate to **API Keys**
3. Click **Create Key**
4. Copy the key — it is only shown once

Keys use the `omega_ent_` prefix. You can create up to 10 active keys per partner. Keys can be set to expire after 30, 90, or 365 days, or never.

## Base URL

```
https://studio.x402omega.cc/api/v1/enterprise
```
...
const BASE = "https://studio.x402omega.cc/api/v1/enterprise";
...
BASE = "https://studio.x402omega.cc/api/v1/enterprise"
...
app.post("/webhooks/omega", (req, res) => {
...
When agents on your platform deploy endpoints, they call the Omega worker API. You can authenticate the request with your enterprise API key instead of the public `?partner=slug` parameter:

```bash
curl -X POST https://api.x402omega.cc/agent/endpoints \
  -H "X-Enterprise-Key: omega_ent_your_api_key_here" \
...
```bash
curl -X PUT "https://api.x402omega.cc/agent/endpoints?slug=my-agent-api&action=topup" \
  -H "X-Enterprise-Key: omega_ent_your_api_key_here" \
  -H "X-Payment: <x402-payment-payload>"
```
...
For agent-native integration, your agents can also connect to the Omega MCP server directly:

**Endpoint:** `https://mcp.x402omega.cc/mcp`
