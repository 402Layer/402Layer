---
title: SDK & Receipts
description: Verify payment receipts with the x402omega SDK for Node.js and Python
---

# SDK & Receipts

Every successful payment through Omega Studio returns a signed **receipt token** (JWT). The x402omega SDK lets you verify these receipts server-side to gate premium content, confirm purchases, or audit payments.

🔗 **GitHub**: [Omega-SDK](https://github.com/gitsafeteam/Omega-SDK)
🔑 **JWKS Endpoint**: [api.x402omega.cc/.well-known/jwks.json](https://api.x402omega.cc/.well-known/jwks.json)
...
```bash
npm install x402omega
```

```js
const { verifyX402ReceiptToken, createX402ReceiptMiddleware } = require('x402omega');
```

### Python

```bash
pip install git+https://github.com/gitsafeteam/Omega-SDK.git#subdirectory=python
```

```python
from x402omega_middleware import X402ReceiptVerifier, require_x402_receipt
```

Dependencies (`PyJWT`, `cryptography`) are installed automatically. For FastAPI support: `pip install x402omega-sdk[fastapi]`.
...
|----------|-------|
| Algorithm | `RS256` |
| JWKS URL | `https://api.x402omega.cc/.well-known/jwks.json` |
| Issuer | `https://api.x402omega.cc` |
| Audience | `x402omega:receipt` |
...
```javascript
const { createX402ReceiptMiddleware } = require('x402omega');
...
```javascript
const { verifyX402ReceiptToken } = require('x402omega');
