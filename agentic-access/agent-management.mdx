...
url = "https://api.x402omega.cc/agent/endpoints"
headers = {
...
const response = await fetch(`https://api.x402omega.cc/agent/endpoints`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'X-Payment': '<signed_payment_proof>'
...
{
  "endpoint": {
    "gateway_url": "https://api.x402omega.cc/e/my-ai-api",
...
<CodeGroup>
```bash cURL
curl -X PUT "https://api.x402omega.cc/agent/endpoints?slug=my-ai-api" \
  -H "X-API-Key: x402_..." \
...
```typescript TypeScript
const response = await fetch(`https://api.x402omega.cc/agent/endpoints?slug=my-ai-api`, {
    method: 'PUT',
    headers: {
        'Content-Type': 'application/json',
        'X-API-Key': 'x402_...',
...
```bash
curl "https://api.x402omega.cc/agent/endpoints?slug=my-ai-api" \
  -H "X-API-Key: x402_..."
```
...
```bash
curl -X DELETE "https://api.x402omega.cc/agent/endpoints?slug=my-ai-api" \
  -H "X-API-Key: x402_..."
```
