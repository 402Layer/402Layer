---
title: Credit-Based Access - Omega Studio
...
**GET** `https://api.x402omega.cc/api/credits/balance`

<CodeGroup>
```bash cURL
curl -X GET "https://api.x402omega.cc/api/credits/balance?endpoint=target-endpoint-slug" \
...
response = requests.get(
    "https://api.x402omega.cc/api/credits/balance",
...
const response = await fetch(
  "https://api.x402omega.cc/api/credits/balance?endpoint=target-endpoint-slug", 
  {
    headers: { "x-wallet-address": "0xAgentAddress..." }
  }
);
