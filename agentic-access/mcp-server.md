...
MCP for Omega Layer. Discover listings, manage endpoints and products, run payment-backed endpoint flows, consume public product and credit-pack purchase flows, and use wallet-session agent registry tools through authenticated MCP actions.
...
The Omega Layer MCP server exposes the broader platform through this protocol, combining marketplace discovery, endpoint and product management, and ERC-8004 agent access in a single MCP surface.
...
```bash
# MCP HTTP Endpoint
https://mcp.x402omega.cc/mcp

# Alternative Cloudflare Worker URL
https://mcp.x402omega.xyz/mcp
```
...
```json
{
  "mcpServers": {
    "omega": {
      "url": "https://mcp.x402omega.cc/mcp"
    }
  }
}
```
...
Manage your x402 endpoints and products through authenticated MCP tools. Use `accessToken` with a scoped `omega_pat_*` token for owner-wide automation. Legacy endpoint keys in the `x402_*` format remain supported where noted.
...
> Security note: PATs and legacy endpoint keys are passed per-request and never stored by the MCP server. Production PATs use `omega_pat_*`, production keys use `x402_*`, legacy `sk_*` keys remain accepted, and older agent-created endpoints without a linked dashboard user stay intentionally constrained to endpoint-only scope.
>
...
| URI | Description |
|-----|-------------|
| `omega://featured` | Featured marketplace listings |
| `omega://top-rated` | Top rated listings |
| `omega://categories` | Available categories |
| `omega://networks` | Supported blockchain networks |
| `omega://agents` | All registered ERC-8004 agents |
| `omega://listing/{slug}` | Individual listing details |
| `omega://agent/{network}/{id}` | Agent details by network and ID |
...
```json
{
  ...
  "arguments": {
    ...
    "accessToken": "omega_pat_your_token_here"
  }
}
```
...
```bash
# Test MCP initialize
curl -X POST https://mcp.x402omega.cc/mcp \
...
- [MCP Endpoint](https://mcp.x402omega.cc)
- [MCP Registry](https://registry.modelcontextprotocol.io/?q=omega)
