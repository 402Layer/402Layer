---
title: x402 Compute Skill - Omega Studio Docs
description: Provision, manage, resize, and extend VPS/GPU compute with x402 or MPP payments, signed auth, API keys, and one-time password fallback
---

# x402 Compute Skill

The `x402-compute` skill lets agents and developers provision and manage VPS/GPU instances on Vultr or DigitalOcean using x402 payments on Base, Solana, or MegaETH, MPP payments through Tempo-compatible clients such as `mppx`, or pre-loaded USD credits. Optional OWS-backed auth and API keys are available for management flows, including in-place resize.
...
<Note>
  Current release: **v1.4.0**. x402 Base/Solana/MegaETH payments are live. Pre-loaded USD credits now available for payment-free provisioning and extending. MegaETH uses USDm (ERC-2612 permit) with an embedded facilitator — gasless for users. MPP Tempo USDC.e payments are also live for agent/CLI provisioning.
</Note>
...
```bash
curl -fsSL https://api.x402omega.cc/skill/x402-compute/install | bash
```
...
- Hosted installer: `https://api.x402omega.cc/skill/x402-compute/install`
